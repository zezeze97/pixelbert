...
class VisualInputEmbedding(nn.Module):
    """
    processing image grid feature -> visual part of input in bert
    """
...
class PixelBertBaseModel(BertPreTrainedModel):
    """
...
class PixelBertForPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.bert = PixelBertBaseModel(config)
...
PixelBertForSequenceClassificationConfig = dict(
...
class PixelBertForSequenceClassification(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(PixelBertForSequenceClassification, self).__init__(config)
        self.config = config
...
class PixelBertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        super(PixelBertForMultipleChoice, self).__init__(config)
        self.config = config
    ...
    def forward(self, text_input_ids, visual_inputs,
        ...
        """
        Args:
            text_input_ids: (B * num_choices, Lt)
            visual_inputs: (B * num_choices, Lv, d)
            text_input_mask: (B * num_choices, Lt)
            labels: (B, ), in [0, num_choices-1]

        Returns:

        """
        outputs = self.bert(
            text_input_ids=text_input_ids, # (B * num_choices, Lt)
            visual_inputs=visual_inputs, # (B * num_choices, Lv, d)
            attention_mask=text_input_mask,  # (B * num_choices, Lt) note this mask is text only!!!
        )
        pooled_output = outputs[1] # (B * num_choices,d)

        pooled_output = self.dropout(pooled_output) # (B * num_choices, d)
    ...
    def calc_loss(self, logits, labels):
        # reshape logits
        if self.config.loss_type == "ce" or self.config.loss_type == "bce":  
            logits = logits.view(-1, self.config.num_labels) # (B, num_choices)
