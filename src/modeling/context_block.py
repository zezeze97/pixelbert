    ...
    def spatial_pool(self, x):
        ...
        if self.pooling_type == 'att':
            ...
            if self.att_scale and self.headers > 1:
                context_mask = context_mask / torch.sqrt(torch.tensor(float(self.single_header_inplanes)))
...
if __name__ == '__main__':
    ...
    net = ContextBlock(inplanes = 128,ratio=0.0625,
                headers=8,
                att_scale=True,
                fusion_type="channel_add",)
    output = net(input)
    summary(net.cuda(),input_size= (128,64,64), batch_size= 32)
